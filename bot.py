...
# تأكد من تعريف bot = commands.Bot(command_prefix='!', intents=intents) قبل هذا الكود

@bot.command(name="استدعاء", description="إرسال طلب استدعاء رسمي إلى عضو معين في الخاص.")
async def summon_embed(ctx, العضو: discord.Member, *, السبب: str):
    """
    يرسل رسالة استدعاء مخصصة وخالية من أي حقوق نشر إلى الخاص
    """
    # الحصول على التاريخ والوقت الحالي وتنسيقهما
    ...
    embed.add_field(name="👤 بواسطة", value=ctx.author.mention, inline=False)
    ...
    # تذييل الرسالة (خالٍ من أي شعارات أو حقوق)
    embed.set_footer(text="في حال عدم الحضور سيتم اتخاذ الإجراءات اللازمة.")

    # --- الجزء الجديد للإرسال في الخاص ---
    try:
        await العضو.send(embed=embed)
        # إرسال تأكيد في القناة الأصلية أن الرسالة تم إرسالها في الخاص
        await ctx.send(f"✅ تم إرسال رسالة الاستدعاء إلى {العضو.mention} في الخاص.")
    except discord.Forbidden:
        # إذا كان العضو قد أغلق الخاص، يتم إرسال الرسالة في القناة العامة
        await ctx.send(f"❌ تعذر إرسال رسالة في الخاص للعضو {العضو.mention}. تم إرسالها هنا بدلاً من ذلك:", embed=embed)
